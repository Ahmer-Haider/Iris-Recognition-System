...
if __name__ == "__main__":
    ...
    excel_file_path = 'excelDataSet.xlsx'  # Path to Excel file
    user_entered_path = input(r"Enter the image path to compare: ")

    # Read image paths from the Excel file
    image_paths, df = read_image_paths_from_excel(excel_file_path)  

    if not image_paths:
        print("No image paths found.")
        exit()

    if user_entered_path == "":
        print("No user entered image path.")
        exit()

    img2 = load_image(user_entered_path)

    if img2 is None:
        print("Error loading user entered image.")
        exit()
    ...
    for idx, image_path in enumerate(image_paths):
        ...
        if pupil_circle1 and ext_iris_circle1 and pupil_circle2 and ext_iris_circle2:
            ...
            if len(matches) > 10 :
                print(f"The irises match for image {idx + 1}.")
                if df.loc[idx, 'CASTED VOTE'] != 1:
                   df.loc[idx, 'CASTED VOTE'] = 1
                else:
                    print("Already Voted Cannot vote")
                break
                                   
                   
               
            else:
                print(f"The irises do not match for image {idx + 1}.")
                df.loc[idx, 'CASTED VOTE'] = 0

        else:
            print(f"Failed to detect iris boundaries for image {idx + 1}.")
            df.loc[idx, 'CASTED VOTE'] = 0  # Indicate failure in detecting boundaries
    ...
    for attempt in range(attempts):
        ...
        print("Failed to save the file after multiple attempts.")
